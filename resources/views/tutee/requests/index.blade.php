            ...
            @foreach ($requests as $request)
                ...
                <div class="group flex flex-col rounded-xl bg-white shadow-sm dark:bg-gray-800">
                    <div class="p-4 md:p-7">
                        ...
                        <div class="flex flex-col justify-center text-center text-white my-2">
                            <span class="bg-blue-400 rounded-full w-full font-bold p-1">{{ucfirst($request->status)}}</span>
                        </div>

                        <div class="mt-4 flex flex-col items-center gap-y-3">
                            <!-- Pay and Chat Accepted or Rejected or Pending (Side by Side) -->
                            <div class="flex gap-x-3">
                                @if ($request->status == 'accepted' && Auth::user()->isTutee())
                                    <!-- Pay Button -->

                                    @if ($request->payment->status == 'success')
                                        <span class="bg-green-400 rounded-full w-full font-bold p-1 px-4">Paid</span>
                                    @else
                                        <a href="{{ route('payments.index', $request->id) }}"
