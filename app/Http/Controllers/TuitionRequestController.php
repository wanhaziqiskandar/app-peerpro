...
class TuitionRequestController extends Controller
{
    ...
    //     // Check if tutee has other sessions at the same time
    //     $tutee_not_available = $user->sentTuitionRequests()
    //         ->where('date', $request_date)
    //         ->where('timeslot', $request_timeslot)
    //         ->whereIn('status', ['pending', 'accepted', 'paid'])
    //         ->exists();

    //     if ($tutee_not_available) {
    //         return redirect(route('requests.index'))
    //             ->with('error', 'You have other session scheduled with the same date and timeslot selected.');
    //     }

    //     // Check if tutor has other sessions at the same time
    //     $tutor_not_available = $tutor->receivedTuitionRequests()
    //         ->where('date', $request_date)
    //         ->where('timeslot', $request_timeslot)
    //         ->whereIn('status', ['accepted', 'paid'])
    //         ->exists();

    //     if ($tutor_not_available) {
    //         return redirect(route('requests.index'))
    //             ->with('error', 'The tutor is not available for the selected date and timeslot.');
    //     }

    //     // Create the tuition request if both checks pass
    //     $request = $user->sentTuitionRequests()->create([
    //         'tutor_id' => $request['tutor_id'],
    //         'expertise' => $request->expertise,
    //         'assessment_id' => $subject->assessment_id ?? null,
